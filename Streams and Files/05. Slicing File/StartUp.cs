...
namespace _05._Slicing_File
{
    public class StartUp
    {
        public static void Main()
        {
            ...
            Slice(@"..\files\sliceMe.mp4", @"..\files\", parts);

            var files = new List<string>();

            for (int i = 1; i <= parts; i++)
            {
                files.Add($@"..\files\part{i}.mp4");
            }
        ...
        private static void Slice(string sourceFile, string destinationDirectory, int parts)
        {
            using (var video = new FileStream(sourceFile, FileMode.Open, FileAccess.Read))
            {
                ...
                for (int i = 1; i <= parts; i++)
                {
                    ...
                    using (var copyVideo = new FileStream($"{destinationDirectory}part{i}{extension}", FileMode.Create, FileAccess.Write))
                    {
                        ...
                        {
                            copyVideo.Write(buffer, 0, buffer.Length);
        ...
        {
            string extension = files.First().Substring(files[0].LastIndexOf("."));
            ...
            string assebled = $"{destinationDirectory}Assebled{extension}";
